...
const SPRINT = {
  ...
  SESSION_TEMPLATES: [
    ...
    { name: "Unilateral + Eccentric Durability", slots: ["lunge_split:sprintTransfer", "knee_flexion_iso:eccentric", "hip_flexors:eccentric", "trunk_antirotation:sprintTransfer"] },
...
function buildHypertrophyWeek(user, exerciseDB, history) {
  ...
  for (const muscle of allMuscles) {
    let sets = HYPERTROPHY.WEEKLY_SETS.default;
  ...
  for (const session of daySessions) {
    for (const [muscle, setBudget] of Object.entries(session.muscleSetBudget)) {
...
function buildSprintWeek(user, exerciseDB, history) {
  ...
  return templates.map((template, i) => {
    ...
    for (const slot of template.slots) {
      ...
      if (session.cnsCostUsed > SPRINT.CNS_BUDGET_PER_SESSION) break; // hard CNS ceiling
